...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `http://localhost:5173/courses/course-progress/${courseId}`,
      cancel_url: `http://localhost:5173/courses/course-detail/${courseId}`,
      metadata: {
        courseId: courseId,
        userId: userId,
      },
...
export const shuriPayment = async (req, res) => {
  try {
    ...
    const transaction = new CoursePurchase({
      ...
      courseId,
      amount,
      paymentId: `PAM_${Date.now()}`,
      transactionId: `TRN_${Date.now()}`,
      status: "completed",
    });
    ...
    res.status(200).json({ message: "Payment Successful & User Enrolled" });
